...
Remember, that we cannot convert only on file level. A while .NET Project is either C# or VB. So, we're convert either _all_ files inside of a project or none.
...
Note, that this library already exist in a different GitHub repo, so, you should create a dedicated branch for that work, so the user can later decide to keep it.
...
### Forms/UserControls without CTor in VB _generate_ the `InitializeComponent()` method call
...
* Convert to SDK-Style project type first, and _only_ keep those settings inside which are really necessary.
