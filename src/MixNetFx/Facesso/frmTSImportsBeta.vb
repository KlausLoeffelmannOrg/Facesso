...
Public Class frmTSImport
    ...
    Private Sub btnImportNow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImportNow.Click
        ...
        If chkTransformBaseData.Checked Then
            'L�schen der Stammdaten
            lblStatus.Text = "L�schen der vorhandenen Daten. Dieser Vorgang kann eine ganze Weile in Anspruch nehmen..."
            lblStatus.Update()
            Try
        ...
        Using locOleDBConnection
            ...
            Catch ex As Exception
                MessageBox.Show("Beim �ffnen der Access-Datenbank ist ein Fehler aufgetreten. Bitte �berpr�fen Sie den Pfad und den Dateinamen zur Access-Datenbank.", _
                                "Fehler beim �ffnen der Datenbank:", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Return
            End Try
            ...
            If chkTransformBaseData.Checked Then
                ...
                Do While locReader.Read
                    ...
                    lblStatus.Text = "Lohngruppen �bernehmen: " + locWageGroup.DisplayName
                    lblStatus.Update()
                    Application.DoEvents()
                Loop

                'Kostenstellen �bernehmen
                ...
                Do While locReader.Read
                    ...
                    If Not locReader.IsDBNull(locReader.GetOrdinal("Kostenstelle")) Then
                        ...
                        If Integer.TryParse(locCostCenterString, locCostCenterInteger) Then
                            ...
                            locCostCenterString = locCostCenterInteger.ToString & ": * Name zu erg�nzen *"
                        Else
                        ...
                        With locCostCenter
                            ...
                            .CostCenterDescription = "Aus anderer Anwendung �bernommen/noch zu erg�nzen"
                            ...
                            .IncentiveWageSynonym = "Pr�mienlohn"
                            .UseFixValuedBonus = False
                            .WasCurrentTo = #12/31/2199#
                        End With
                        lblStatus.Text = "Kostenstellen �bernehmen: " + locCostCenter.DisplayName
                        lblStatus.Update()
                        Application.DoEvents()
                ...
                Do While locReader.Read
                    ...
                    With locAdrDetails
                        ...
                        .Street = ADDBNullable.FromObject(Of String)(locReader.GetValue(locReader.GetOrdinal("Stra�e")))
                        .Zip = ADDBNullable.FromObject(Of String)(locReader.GetValue(locReader.GetOrdinal("PLZ")))
                    End With
                    lblStatus.Text = "Mitarbeiter �bernehmen: " + locEmployee.DisplayName
                    lblStatus.Update()
                    Application.DoEvents()
                ...
                Do While locReader.Read
                    ...
                    With locLabourValue
                        ...
                        End Try
                    End With
                    lblStatus.Text = "Arbeitswerte �bernehmen: " + locLabourValue.DisplayName
                    lblStatus.Update()
                    Application.DoEvents()
                ...
                Do While locReader.Read
                    ...
                    End With
                    lblStatus.Text = "Arbeitsgruppen �bernehmen: " + locWorkGroup.DisplayName
                    lblStatus.Update()
                    Application.DoEvents()
                ...
                For Each locWorkGroup In locWorkGroups
                    locCommand = New OleDbCommand("SELECT Arbeitswertnr FROM AGrpArbeitswertDef WHERE ArbeitsgruppenNr=? ORDER BY OrdinalNr", locOleDBConnection)
                    locCommand.Parameters.AddWithValue("@ArbeitsgruppenNr", locWorkGroup.WorkGroupNumber)
                    locReader = locCommand.ExecuteReader()
                    ...
                    End If
                    lblStatus.Text = "Arbeitsgruppenzuordnung vornehmen f�r: " + locWorkGroup.DisplayName
                    lblStatus.Update()
                    Application.DoEvents()
                Next
            End If
            ...
            If chkTransformProductionData.Checked Then
                'Mengendaten �bernehmen
                locCommand = New OleDbCommand("SELECT ArbeitsgruppenNr, Tagesdatum, Schicht, ArbeitswertNr, Menge FROM [AgrpMengenerfassung] " & _
                        "WHERE Tagesdatum>=? ORDER BY Tagesdatum, Schicht, ArbeitsgruppenNr", locOleDBConnection)
                locCommand.Parameters.AddWithValue("@Tagesdatum", myTransformFrom.Date)
                locReader = locCommand.ExecuteReader()
                ...
                Do While locReader.Read
                    ...
                    If Not locFirst Then
                        ...
                        locPd.Clear()
                        lblStatus.Text = "Mengendaten �bernehmen: " + locProductionDate.ToShortDateString + "; S:" & locShift & " - " & locPd.WorkGroup.ListItemText
                        lblStatus.Update()
                    ...
                    Else
                        If locPd.ProductionDate <> locProductionDate Or _
                           ...
                           locPd.WorkGroup.IDWorkGroup <> locWorkGroup.IDWorkGroup Then
                            ...
                            lblStatus.Text = "Mengendaten �bernehmen: " + locProductionDate.ToShortDateString + "; S:" & locShift & " - " & locWorkGroup.ListItemText
                            lblStatus.Update()
                            Application.DoEvents()
                            locPd = New ProductionData()
                            locPd.ProductionDate = locProductionDate
            ...
            If chkTransformEmployeeTimes.Checked Then
                ...
                locCommand.Parameters.AddWithValue("@PersTagesdatum", myTransformFrom.Date)
                locReader = locCommand.ExecuteReader()
                Dim locFirst As Boolean = False
                ...
                Do While locReader.Read
                    ...
                    If Not locFirst Then
                        ...
                        locTlis.Clear()
                        lblStatus.Text = "Mitarbeiterzeiten �bernehmen: " + locProductionDate.ToShortDateString + "; S:" & locShift & " - " & locTlis.WorkGroup.ListItemText
                        lblStatus.Update()
                        locFirst = True
                    Else
                        If locTlis.ProductionDate <> locProductionDate Or _
                           ...
                           locTlis.WorkGroup.IDWorkGroup <> locWorkGroup.IDWorkGroup Then
                            ...
                            lblStatus.Text = "Mitarbeiterzeiten �bernehmen: " + locProductionDate.ToShortDateString + "; S:" & locShift & " - " & locWorkGroup.ListItemText
                            lblStatus.Update()
                            Application.DoEvents()
                            locTlis = New EmployeeTimeLogInfo()
                            locTlis.ProductionDate = locProductionDate
        ...
        lblStatus.Text = "Die �bernahme wurde erfolgreich durchgef�hrt!"
        If myProtocol <> "" Then
            MessageBox.Show(myProtocol, "Unregelm��igkeiten bei der �bernahme:")
        End If
        lblStatus.Update()
...
Public Class RandomData
    ...
    Shared Sub New()
        ...
        myLastnames = New String() {"Heckhuis", "L�ffelmann", "Thiemann", "M�ller", _
        ...
        myFirstnames = New String() {"J�rgen", "Gabriele", "Uwe", "Katrin", "Hans", _
    ...
    Public Shared Function DistortOrgNames(ByVal Text As String) As String
        ...
        Text = Text.Replace("B�renbach", "W�nnenberg")
