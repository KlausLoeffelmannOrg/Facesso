        ...
        internal WorkGroupInfo GetWorkGroup(int idSubsidiary, int idWorkGroup)
        {
            ...
            using (locConnection)
            {
                var locCommand = new SqlCommand(
                    ...
                    "[WorkGroups].[IDSubsidiary]=@IDSubsidiary AND [WorkGroups].[IsCurrent]='true' AND [WorkGroups].[IDWorkGroup]=@IDWorkGroup",
                    locConnection);
        ...
        internal WorkGroupInfo GetWorkGroupByWorkGroupNumber(int idSubsidiary, int workGroupNumber)
        {
            ...
            using (locConnection)
            {
                var locCommand = new SqlCommand(
                    ...
                    "[WorkGroups] INNER JOIN [CostCenters] ON " +
                    "[WorkGroups].[IDCostCenter] = [CostCenters].[IDCostCenter] WHERE " +
                    "[WorkGroups].[IDSubsidiary]=@IDSubsidiary AND [WorkGroups].[IsCurrent]=1 AND [WorkGroups].[WorkGroupNumber]=@WorkGroupNumber",
                    locConnection);
                locCommand.Parameters.Add("@IDSubsidiary", SqlDbType.Int).Value = FacessoGeneric.LoginInfo.IDSubsidiary;
