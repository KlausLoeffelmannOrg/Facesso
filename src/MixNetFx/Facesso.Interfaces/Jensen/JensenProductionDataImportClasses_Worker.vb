    ...
    Private Sub retrieveDataForDate(ByVal ProductionDate As Date)
        ...
        Using locConnection
            Dim locAdapter As New SqlDataAdapter(
                "SELECT * FROM [LogData] WHERE " & _
                "(TargetId=@TargetId) AND " & _
                "([TimeStamp] >= @FromDate AND [TimeStamp] <= @ToDate) AND " & _
                "(([Mode]=1 AND [ValueID]=1) OR [Mode]=6)" & _
                " ORDER BY [TargetID],[LineNo],[Mode]", locConnection)
            locAdapter.SelectCommand.Parameters.Add("@TargetId", SqlDbType.NVarChar, 100).Value = Me.JensenDeviceID
            locAdapter.SelectCommand.Parameters.Add("@FromDate", SqlDbType.DateTime).Value = ProductionDate.Date
            locAdapter.SelectCommand.Parameters.Add("@ToDate", SqlDbType.DateTime).Value = ProductionDate.Date.AddDays(1).AddSeconds(-1)
        ...
        For Each locDataRow As DataRow In locSourceData.Rows
            ...
            Try
                ...
                If CInt(locDataRow("Mode")) = 1 And CInt(locDataRow("ValueID")) = 1 Then
                    'Ja, neue Programmnr. f�r die n�chsten Datens�tze
                    locCurrPrgNo = CInt(locDataRow("Value"))
                    'und n�chster Datensatz
                    Continue For
                End If
