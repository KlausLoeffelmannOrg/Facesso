CREATE PROCEDURE dbo.TimeLog_GetLogItemsForShiftDate
	...
	WHERE     (TimeLog.IDSubsidiary = @IDSubsidiary) AND (TimeLog.IDWorkGroup = @IDWorkGroup) AND (TimeLog.ProductionDate = @ProductionDate) AND 
	                      (TimeLog.Shift = @Shift)
	ORDER BY Employees.PersonnelNumber, TimeLog.ShiftStart
