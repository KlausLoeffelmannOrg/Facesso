...
{
    public class FacPrintWorkGroupListShiftWiseCompressed : FacessoPrintBase
    {
        private WorkGroupAnalysisInfoItems myWorkGroupAnalysis;
        private ProductionPeriod myProductionPeriod;
        public FacPrintWorkGroupListShiftWiseCompressed(WorkGroupAnalysisInfoItems WorkGroupAnalysis, ProductionPeriod Period, string Username) : base("Produktiv-Site-Auswertung", WorkGroupAnalysis.Period.RangeDescription, Username)
        {
        ...
        protected override void PrepareDocument()
        {
            ...
            double locTotalWorkingTime = default(double);
            {
                ...
                __with0.BeginTable(BorderStyle, 75, 75, 180, 140, 150, 125);
                __with0.BuildTableHeader();
                __with0.CurrentCellAlignment = ActiveDev.Printing.ADTextCellAlignment.TopCenter;
                __with0.WriteCells("Schicht", "Site-Nr.:", "Site-Name", "Unterbr.-" + System.Environment.NewLine + "zeiten", "Netto-" + System.Environment.NewLine + "zeiten", "Kennzahlen");
                ...
                foreach (WorkGroupAnalysisInfo locAnalysisInfo in myWorkGroupAnalysis)
                {
                    for (int locShift = 1; locShift <= 4; locShift++)
                    {
                        ...
                        if (locDoPrint & locItem != null)
                        {
                            ...
                            __with0.CurrentCellAlignment = ActiveDev.Printing.ADTextCellAlignment.TopRight;
                            __with0.WriteCell("Ausfall: " + locItem.TotalDownTime.ToString("#,##0") + " Min.");
                            __with0.CurrentCellAlignment = ActiveDev.Printing.ADTextCellAlignment.TopCenter;
                            __with0.WriteCell("Effektiv (ang.): " + locItem.TotalEffectiveIWTAdj.ToString("#,##0") + " Min.");
                            __with0.CurrentCellAlignment = ActiveDev.Printing.ADTextCellAlignment.TopCenter;
                            __with0.WriteCell("Zeitgrad (angp.): " + locItem.DegreeOfTimeAdj.ToString("##0"));
